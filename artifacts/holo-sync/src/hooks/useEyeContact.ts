...
export function useEyeContact(face: FaceDetectionData): EyeContactData {
  ...
  useEffect(() => {
    ...
    if (!face.detected || !face.box || face.keypoints.length < 6) {
      setData(prev => ({
        ...prev,
        isLooking: false,
        score: 0,
    ...
    const vw = face.videoW || 640;
    const vh = face.videoH || 480;
    const box = face.box;
  ...
  }, [face.detected, face.box, face.keypoints, face.videoW, face.videoH]);
