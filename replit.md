...
### Features
- **rPPG Heartbeat Detection (VitalLens)**: Powered by Rouast Labs' VitalLens.js library (v0.4.5, loaded from CDN at runtime). Uses `method: "pos"` — the POS algorithm running fully locally in the browser with no API key required. Research-grade accuracy (1.57 bpm MAE on test set). VitalLens handles its own face detection, ROI extraction, and PPG signal processing. Results are EMA-smoothed (α=0.3) with adaptive jump filtering. PPG waveform visualization from VitalLens output. Stress classification based on BPM thresholds (low < 83, medium < 100, high ≥ 100).
