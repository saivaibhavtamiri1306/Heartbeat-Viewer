...
### Features
- **rPPG Heartbeat Detection (v5)**: Research-grade multi-algorithm pipeline. Three methods run in parallel: POS (Wang et al. 2017), CHROM (De Haan & Jeanne 2013), and GREEN channel FFT — best selected by spectral SNR. Pipeline: skin-filtered ROI sampling (YCbCr per-pixel validation) → motion artifact detection & frame rejection → signal detrending (moving-mean removal) → Butterworth bandpass (0.75–3.0 Hz) → 4x zero-padded Hamming-windowed FFT → parabolic peak interpolation → harmonic/sub-harmonic rejection → autocorrelation cross-validation → IQR + MAD outlier rejection on 25-sample history → quality-adaptive EMA smoothing. BUFFER_SIZE=360, MIN_FRAMES=90, processes every 3rd frame. Skin ratio tracking for ROI quality.
