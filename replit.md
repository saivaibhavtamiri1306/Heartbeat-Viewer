...
### Features
- **rPPG Heartbeat Detection (v6)**: Research-grade multi-algorithm pipeline. Three methods run in parallel: POS (Wang et al. 2017), CHROM (De Haan & Jeanne 2013), and GREEN channel FFT — best selected by spectral SNR. Pipeline: skin-filtered ROI sampling (YCbCr per-pixel validation) → illumination normalization (bounded gain, per-channel clamping) → motion artifact detection & frame rejection → signal detrending (moving-mean removal) → Butterworth bandpass (0.75–3.0 Hz) → Welch's method PSD (power-averaged segmented periodogram with 50% overlap, Hamming windowed, tail-segment coverage) for buffers ≥90 frames OR 4x zero-padded FFT for shorter buffers → parabolic peak interpolation → harmonic/sub-harmonic rejection → autocorrelation cross-validation → consensus voting (SNR-weighted average when 2+ algorithms agree, +15 confidence bonus) → IQR + MAD outlier rejection on 25-sample history → quality-adaptive EMA smoothing. Weighted ROI sampling (forehead 3x, cheeks 2x, nose 1.5x). BUFFER_SIZE=360, MIN_FRAMES_FAST=64, MIN_FRAMES_FULL=90, processes every 3rd frame.
...
- Design: Calming palette — bg gradient #060e1a→#081420→#0a1828, teal #4ecdc4, lavender #a78bfa, warm amber #ffc078, soft red #ff6b6b. Borders rgba(78,205,196,0.06-0.12). Font Orbitron. maxQuestions=15.
