...
### Features
- **rPPG Heartbeat Detection (v3)**: Research-grade webcam heart rate based on Wang 2017 POS + Tarvainen 2002 detrending + pyVHR/heartbeat-js pipelines. Key: narrowed bandpass to 0.67–2.5 Hz (40–150 BPM) to eliminate harmonics; explicit harmonic rejection (checks f/2 and f/3 subharmonics in FFT); λ=10 detrending (correct per Tarvainen 2002 at 30fps); 4th-order Butterworth (two cascaded biquads); Tukey window; 256-sample Welch segments; 10s window (WIN=300); ACF first-peak preference; median-of-5 output smoothing; EMA α=0.15 for stability.
